...
class Generator_Layer(Layer):
    ...
    def feed_forward_lrelu(self, input_data):
        output = tf.nn.relu(tf.add(tf.matmul(input_data, self.weights), self.biases))
        return output
...
class Discriminator_Layer(Layer):
    ...
    def feed_forward_1(self, input_data):
        output = tf.nn.dropout(tf.nn.relu(tf.add(tf.matmul(input_data, self.weights), self.biases)), keep_prob=0.3)
        return output
