...
class MODEL():
    ...
    def __init__(self):
        ...
        self.dropout = tf.placeholder(dtype = tf.float32, name = 'drop_out')
    ...
    def Generator(self, Z):
        ...
        hidden1 = neural_network.Generator_Layer(shape = [dim, 256], name = "gn_1", stddev = 0.02, value = 0.)
        h1 = hidden1.feed_forward_lrelu(Z)
        
        hidden2 = neural_network.Generator_Layer(shape = [256, 512], name = "gn_2", stddev = 0.02, value = 0.)
        h2 = hidden2.feed_forward_lrelu(h1)
        
        hidden3 = neural_network.Generator_Layer(shape = [512, 1024], name = "gn_3", stddev = 0.02, value = 0.)
        h3 = hidden3.feed_forward_lrelu(h2)
        
        hidden4 = neural_network.Generator_Layer(shape = [1024, 784], name = "gn_4", stddev = 0.02, value = 0.)
        h4 = hidden4.feed_forward_tanh(h3)
        
        return h4
    
    def Discriminator(self, x, dropout):
        dim = x.get_shape()[1].value
        hidden1 = neural_network.Discriminator_Layer(shape = [dim, 1024], name = "dis_1", stddev = 0.02, value = 0.)
        h1 = hidden1.feed_forward_1(x, dropout)
        
        hidden2 = neural_network.Discriminator_Layer(shape = [1024, 512], name = "dis_2", stddev = 0.02, value = 0.)
        h2 = hidden2.feed_forward_1(h1, dropout)
        
        hidden3 = neural_network.Discriminator_Layer(shape = [512, 256], name = "dis_3", stddev = 0.02, value = 0.)
        h3 = hidden3.feed_forward_1(h2, dropout)
        
        hidden4 = neural_network.Discriminator_Layer(shape = [256, 1], name = "dis_4", stddev = 0.02, value = 0.)
        h4 = hidden4.feed_forward_2(h3, dropout)
    ...
    def train(self, data, gen_out):
        ...
        gen_optimizer = tf.train.AdamOptimizer(config.LEARNING_RATE, beta1 = 0.5).minimize(self.gen_loss, var_list = gen_vars)
        dis_optimizer = tf.train.AdamOptimizer(config.LEARNING_RATE, beta1 = 0.5).minimize(self.dis_loss, var_list = dis_vars)
        ...
        with tf.Session(config=tf.ConfigProto(log_device_placement=True)) as session:
            ...
            for epoch in range(config.NUM_EPOCHS):
                ...
                for batch in range(int(data.size/config.BATCH_SIZE)):
                    ...
                    #discriminator step
                    feed_dict = {self.dis_input: batchX, self.gen_input: z, self.dropout: 0.3}
                    dis_loss_, _ = session.run([self.dis_loss, dis_optimizer], feed_dict = feed_dict)
                    D_losses.append(dis_loss_)
                    
                    #generator step
                    z = np.random.normal(0, 1, (config.BATCH_SIZE, 100))
                    feed_dict = {self.gen_input: z, self.dropout: 0.3}
                ...
                if epoch % 10 == 0:
                    self.test(gen_out, session, epoch)
            ...
            utils.plot(self.gen_losses, self.dis_losses)
            self.test(gen_out, session, epoch)
            
    def test(self, gen_out, session, epoch):
        # saver = tf.train.Saver()
        # with tf.Session(config=tf.ConfigProto(log_device_placement=True)) as session:
        # saver.restore(session, os.path.join(config.MODEL_DIR, "model" + str(config.BATCH_SIZE) + "_" + str(config.NUM_EPOCHS) + ".ckpt"))
        z = np.random.normal(0, 1, (config.TEST_SAMPLES, 100))
        feed_dict = {self.gen_input: z, self.dropout: 0.0}
