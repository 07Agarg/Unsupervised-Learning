...
class Encoder(nn.Module):
    ...
    def __init__(self):
        ...
        self.enc = nn.Linear(in_features=128*10*10,
                             out_features=config.LATENT_DIM)

    def forward(self, x):
        x = F.dropout(F.leaky_relu(self.bn1(self.conv1(x))),
                      p=config.DROPOUT)
        x = F.dropout(F.leaky_relu(self.bn2(self.conv2(x))),
                      p=config.DROPOUT)
        x = F.dropout(F.leaky_relu(self.bn3(self.conv3(x))),
                      p=config.DROPOUT)
